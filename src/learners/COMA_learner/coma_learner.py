...
class COMALearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.log_alpha = th.tensor(np.log(0.01), dtype=th.float)
        ...
        self.agent_optimiser = Adam(params=self.agent_params, lr=args.lr, betas=[args.optim_alpha,0.999], eps=args.optim_eps)
        self.critic_optimiser = Adam(params=self.critic_params, lr=args.critic_lr, betas=[args.optim_alpha,0.999], eps=args.optim_eps)
        self.log_alpha_optimiser = Adam(params=[self.log_alpha], lr=args.critic_lr, betas=[args.optim_alpha,0.999], eps=args.optim_eps)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        q_vals, critic_train_stats = self._train_critic(batch, rewards, terminated, actions, avail_actions,
                                                        critic_mask, bs, max_t,target_pi)
        ...
        entropy = -th.sum(pi_taken * log_pi_taken , dim=1, keepdim=True)
        ...
        # 更新温度系数alpha
        alpha_loss = th.mean((mean_entropy-self.target_entropy).detach() * self.log_alpha.exp())
        # 梯度更新
    ...
    def _train_critic(self, batch, rewards, terminated, actions, avail_actions, mask, bs, max_t,target_pi):
        ...
        targets = build_td_lambda_targets(rewards, terminated, mask, targets_taken, self.n_agents, self.args.gamma, target_pi,self.log_alpha)
        ...
        for t in reversed(range(rewards.size(1))):
            ...
            q_taken = q_t.mean(dim=3).squeeze(3)
            targets_t = targets[:, t]
