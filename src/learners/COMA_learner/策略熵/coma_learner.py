...
class COMALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        policy_entropy = (pi_taken * log_pi_taken).sum(-1).mean()
        #print('\n policy entropy:''{:f}'.format(policy_entropy))
        advantages = (q_taken - baseline).detach()

        coma_loss = - (((advantages * log_pi_taken) * mask).sum() + 0.001 * policy_entropy)/ mask.sum()
        #coma_loss_s = - ((advantages * log_pi_taken) * mask).sum() / mask.sum()
        #print('\n coma loss:''{:f}'.format(coma_loss))
        #print('\n coma loss_s:''{:f}'.format(coma_loss_s))
        # Optimise agents
